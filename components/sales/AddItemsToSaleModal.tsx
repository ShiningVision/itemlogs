...
export function AddItemsToSaleModal({
  ...
  async function handleAdd() {
    ...
    try {
      await Promise.all(
        selectedIds.map((id) =>
          fetch(`/api/v1/sales/${saleId}/items`, {
            method: 'POST',
    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--spacing-md)', gap: 'var(--spacing-md)', flexWrap: 'wrap' }}>
          <h2 style={{ fontWeight: 'var(--font-weight-bold)' }}>{t('addItems')}</h2>
          <input
            type="text"
        ...
        <div style={{ overflowY: 'auto', flex: 1, marginBottom: 'var(--spacing-md)' }}>
          {items.length === 0 ? (
            <p style={{ color: 'var(--color-text-muted)' }}>{t('noAvailableItems')}</p>
